    ...
    if (solver_flag == INITIAL)
    {
        ...
        if (!STEREO && USE_IMU)
        {
            ...
            if (frame_count == WINDOW_SIZE)
            {
                ...
                // ESTIMATE_EXTRINSIC表示相机外参估计标志（IMU和相机之间的位姿），不等于2说明外参已经知道了
                ...
                if(result)
                {
                    ...
                    // 移动滑窗，更新特征点的观测帧集合、观测帧索引（在滑窗中的位置）、首帧观测帧和深度值，删除没有观测帧的特征点
                    // 滑窗会移除滑窗首帧，这样会让frame_count少一个，下次还能进到这个初始化中
                    slideWindow();
            }
        }
        ...
        if(STEREO && !USE_IMU)
        {
            ...
            // 将新的特征点进行三角化得到逆深度
            f_manager.triangulate(frame_count, Ps, Rs, tic, ric);
            // 后端优化的主要的函数(因为没有IMU，所以也就是优化位姿，逆深度)
            optimization();
...
bool Estimator::relativePose(Matrix3d &relative_R, Vector3d &relative_T, int &l)
{
    ...
    for (int i = 0; i < WINDOW_SIZE; i++)
    {
        ...
        if (corres.size() > 20)
        {
            ...
            // 460 是在虚拟焦距的相机 可以参考https://github.com/HKUST-Aerial-Robotics/VINS-Mono/issues/48
...
void Estimator::gxt_optimization()
{
  ...
  // debug value change
    ...
    auto gxt_loss_function = std::shared_ptr<gxt::LossFunction>(new gxt::HuberLoss(1.0));
    //loss_function = new ceres::CauchyLoss(1.0 / FOCAL_LENGTH);
    //ceres::LossFunction* loss_function = new ceres::HuberLoss(1.0);
    ...
    for (auto &it_per_id : f_manager.feature)
    {
        ...
        for (auto &it_per_frame : it_per_id.feature_per_frame)
        {
            ...
            if (imu_i != imu_j)
            {
               ...
               gxt_f_td->SetLossFunction(gxt_loss_function);
            ...
            {                
                ...
                if(imu_i != imu_j)
                {
                   ...
                   gxt_f->SetLossFunction(gxt_loss_function);
